        ...
        for (int y = 0; y < O; y++){
          ...
          for (int c = 0; c < F; c++){
            for (int r = 0; r < F; r++){
              ...
              //output[o_index] += input[i_index] * weight[w_index] * con;
              one_out += ((input[i_index] == weight[w_index]) ? 1.0 : 0.0); //XNOR
              mac_num++;
              }
            }
          }
          output[o_index] += (2.0 * one_out - mac_num)*con;  //all not fixed points??
        }
      }
    }
  }
